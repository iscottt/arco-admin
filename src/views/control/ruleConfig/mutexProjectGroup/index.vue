...
<script lang="ts" setup>
  ...
  import {
    getRuleMutexProjectPage,
    insertRuleMutexProject,
    updateRuleMutexProject,
    deleteRuleMutexProject,
  } from '@/api/ruleConfig';
    ...
    if (!errors) {
      ...
      try {
        if (modalType.value === 'add') {
          await insertRuleMutexProject(params);
        } else {
          await updateRuleMutexProject(params);
        }
      } catch (error) {
        return false;
      }
  ...
  const fetchData = async (params: any = { startPage: 1, pageSize: 10 }) => {
    setLoading(true);
    try {
      const data: any = await getRuleMutexProjectPage(params);
  ...
  const handleDelete = async (groupId: string) => {
    await deleteRuleMutexProject({ seqIds: groupId.toString() });
    Message.success('操作成功！');
    fetchData({
      startPage: basePagination.current,
      pageSize: basePagination.pageSize,
