...
<script lang="ts" setup>
  ...
  import {
    searchMutexProjectDtl,
    addMutexProjectDtl,
    updateMutexProjectDtl,
    deleteMutexProjectDtl,
  } from '@/api/ruleConfig';
    ...
    if (!errors) {
      ...
      try {
        if (modalType.value === 'add') {
          await addMutexProjectDtl(params);
        } else {
          await updateMutexProjectDtl(params);
        }
      } catch (error) {
        return false;
      }
  ...
  const fetchData = async (params: any = { startPage: 1, pageSize: 10 }) => {
    ...
    try {
      const data: any = await searchMutexProjectDtl({
        ...params,
        groupId: props.groupId,
      });
  ...
  const handleDelete = async (seqId) => {
    await deleteMutexProjectDtl({ seqId: seqId });
    Message.success('操作成功！');
    fetchData({
