  ...
  <div>
    ...
    >
      ...
      >
        <a-row :gutter="16">
          <a-col :span="24">
            <a-form-item field="dependChargeCode" label="收费项目">
              <a-select
                allow-search
                v-model="formModel.dependChargeCode"
                placeholder="请选择"
                @search="handleSearch"
                :filter-option="false"
              >
...
<script lang="ts" setup>
  ...
  const formModel = ref<{ dependChargeCode?: string }>({});
  ...
  const formRules = {
    dependChargeCode: { required: true, message: '收费项目不能为空' },
  };
  /**
   * 提交表单
   * @param done
    ...
    if (!errors) {
      ...
      try {
        if (modalType.value === 'add') {
          await addGroupDetail(params);
        } else {
          await updateGroupDetail(params);
        }
      } catch (error) {
        return false;
      }
  ...
  const fetchData = async (params: any = { startPage: 1, pageSize: 10 }) => {
    ...
    try {
      const data: any = await searchGroupDetail({
        ...params,
        groupId: props.groupId,
      });
  ...
  const handleEdit = async (record) => {
    ...
    const options = [
      {
        chargeCode: record.dependChargeCode,
        chargeName: record.dependChargeName,
      },
    ];
