  ...
  <div>
    ...
    >
      ...
      >
        <a-row :gutter="16">
          <a-col :span="24">
            <a-form-item field="ybChargeKind" label="医保大类">
              <a-input
...
<script lang="ts" setup>
  ...
  const formModel = ref<{ ybChargeKind?: string }>({});
  ...
  const formRules = {
    ybChargeKind: { required: true, message: '医保大类不能为空' },
  };
  /**
   * 提交表单
   * @param done
    ...
    if (!errors) {
      ...
      try {
        if (modalType.value === 'add') {
          await addDependKindDtl(params);
        } else {
          await updateDependKindDtl(params);
        }
      } catch (error) {
        return false;
      }
  ...
  const fetchData = async (params: any = { startPage: 1, pageSize: 10 }) => {
    ...
    try {
      const data: any = await searchDependKindDtl({
        ...params,
        groupId: props.groupId,
      });
