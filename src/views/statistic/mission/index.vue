...
<script lang="ts" setup>
  ...
  import { searchMission, exportMission } from '@/api/statistic';
  import { columns } from './column';
  import { ResultSearchProps } from './interface';
  import { dateTypeList } from '../common';
  ...
  const handleExport = async () => {
    ...
    try {
      const result = await exportMission({
        ...searchForm.value,
        startPage: basePagination.current,
        pageSize: basePagination.pageSize,
      });
  ...
  const fetchData = async (params: any = { startPage: 1, pageSize: 10 }) => {
    setLoading(true);
    try {
      const data: any = await searchMission(params);
